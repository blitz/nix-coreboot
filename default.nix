...
in rec {
  ...
  corebootToolchainTest =
    pkgs.runCommand "coreboot-toolchain-test" {
      nativeBuildInputs = [ corebootToolchain ];
    } ''
      echo "int main() { return 0; }" > test-gcc.c
      i386-elf-gcc test-gcc.c -o test-gcc
      ./test-gcc
    '';

  coreboot = pkgs.stdenvNoCC.mkDerivation {
    pname = "coreboot";
    ...
    postPatch = ''
      ...
      sed -i 's,die "no host compiler found",HOSTCC=${pkgs.gcc}/bin/gcc,' util/xcompile/xcompile
    '';

    # TODO We can use make crossgcc-i386 DEST=$out to turn the cross
    # compiling toolchain into a normal derivation. This would allow
    # building coreboot without the chroot and separate coreboot
    # building (fast) from building the toolchain (slow).
    buildPhase = ''
      cp ${./config} .config
